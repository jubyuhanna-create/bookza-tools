...
const MAP_IMAGES = {
  'Antarctic Peninsula': 'assets/maps/Antarctic Peninsula.jpg',
  'Blizzard World': 'assets/maps/Blizzard World.jpg',
  'Busan': 'assets/maps/Busan.jpg',
  'Circuit Royal': 'assets/maps/Circuit Royal.jpg',
  'Colosseo': 'assets/maps/Colosseo.jpg',
  'Dorado': 'assets/maps/Dorado.jpg',
  'Eichenwalde': 'assets/maps/Eichenwalde.jpeg',
  'Esperança': 'assets/maps/Esperanca.jpeg',
  'Havana': 'assets/maps/Havana.jpg',
  'Hollywood': 'assets/maps/Hollywood.jpg',
  'Ilios': 'assets/maps/Ilios.jpg',
  'Junkertown': 'assets/maps/Junkertown.png',
  "King's Row": 'assets/maps/Kings Row.jpg',
  'Lijiang Tower': 'assets/maps/Lijiang Tower.jpeg',
  'Midtown': 'assets/maps/Midtown.jpg',
  'Nepal': 'assets/maps/Nepal.jpg',
  'New Junk City': 'assets/maps/New Junk City.jpg',
  'New Queen Street': 'assets/maps/New Queen Street.jpg',
  'Numbani': 'assets/maps/Numbani.jpg',
  'Oasis': 'assets/maps/Oasis.jpg',
  'Paraiso': 'assets/maps/Paraiso.jpg',
  'Rialto': 'assets/maps/Rialto.jpg',
  'Route 66': 'assets/maps/Route 66.png',
  'Runasapi': 'assets/maps/Runasapi.jpg',
  'Samoa': 'assets/maps/Samoa.jpg',
  'Shambali Monastery': 'assets/maps/Shambali Monastery.jpg',
  'Suravasa': 'assets/maps/Suravasa.jpg',
  'Watchpoint: Gibraltar': 'assets/maps/Watchpoint Gibraltar.jpg'
};
...
function openMapPicker() {
  ...
  Object.entries(MAPS).forEach(([mode, maps]) => {
    ...
    sorted.forEach(map => {
      const item = document.createElement('button');
      item.type = 'button';
      const img = MAP_IMAGES[map];
      if (img) {
        item.className = 'modal-hero';
        item.innerHTML = `<img src="${img}" alt="${map}" style="width:80px;height:45px;border-radius:6px;object-fit:cover;border:2px solid var(--border)"><div class="modal-hero-name">${map}</div>`;
      } else {
        item.className = 'modal-hero';
        item.innerHTML = `<div style="width:80px;height:45px;border-radius:6px;background:var(--bg-hover);display:grid;place-items:center;font-size:10px;color:var(--text-secondary);border:1px solid var(--border)">No img</div><div class="modal-hero-name">${map}</div>`;
      }
      item.addEventListener('click', () => {
        state.map = map;
        closeModal();
...
function updateButtons() {
  if (state.map) {
    const img = MAP_IMAGES[state.map];
    if (img) {
      dom.mapBtn.innerHTML = `<img src="${img}" alt="${state.map}" style="width:24px;height:16px;border-radius:3px;object-fit:cover;display:inline-block;vertical-align:middle;margin-right:6px">${state.map}`;
    } else {
      ...
      <article class="best-card highlight oq-pick-card perk-selected" data-hero="${selected.hero.name}">
        <div class="result-hero">
          <div class="name-stack">
            <img src="${selected.hero.image}" alt="${selected.hero.name}">
...
function renderPlayerInfo() {
  ...
  const banHtml = banTargets.length
    ? banTargets.map(b => `
        <div class="player-ban-item">
          <img src="${b.hero.image}" alt="${b.hero.name}">
          <div>
  ...
  dom.playerInfo.querySelectorAll('img').forEach(img => hydrateImage(img, img.alt, HERO_BY_NAME[img.alt]?.role));
}
