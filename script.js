...
const MAPS = {
  ...
  Control: ['Busan', 'Ilios', 'Lijiang Tower', 'Nepal', 'Oasis', 'Samoa'],
...
function cacheDom() {
  ...
  dom.playerInfo = document.getElementById('player-hero-info');
}
...
function openMapPicker() {
  ...
  dom.modalCopy.textContent = 'Pick a map to get mode-specific hero recommendations.';
  dom.modalList.innerHTML = '';
  ...
  Object.entries(MAPS).forEach(([mode, maps]) => {
    ...
    sorted.forEach(map => {
      ...
      item.type = 'button';
      item.className = 'modal-hero';
      item.innerHTML = `<div class="modal-hero-name">${map}</div>`;
      item.addEventListener('click', () => {
        state.map = map;
        closeModal();
        updateAll();
...
function updateButtons() {
  if (state.map) {
    dom.mapBtn.textContent = state.map;
  } else {
    dom.mapBtn.textContent = 'Choose a map';
  }
  ...
  dom.enemyCount.textContent = `${getEnemyNames().length}/5`;
  dom.playerCount.textContent = `${state.playerHero ? 1 : 0}/1`;
}
...
function renderBestPick() {
  ...
  state.topHeroes = scored.slice(0, 4);
  if (!state.selectedPerkHero || !state.topHeroes.find(h => h.hero.name === state.selectedPerkHero)) {
    state.selectedPerkHero = state.topHeroes[0].hero.name;
  }

  const selected = state.topHeroes.find(h => h.hero.name === state.selectedPerkHero);
  const alts = state.topHeroes.filter(h => h.hero.name !== state.selectedPerkHero);
  ...
  dom.results.querySelectorAll('.oq-pick-card').forEach(card => {
    ...
    card.addEventListener('click', () => {
      state.selectedPerkHero = card.dataset.hero;
      renderBestPick();
    });
  });

  renderPerks(HERO_BY_NAME[state.selectedPerkHero], enemies, state.map);
}

function renderAlt(item) {
  return `<article class="alt-card oq-pick-card ${state.selectedPerkHero === item.hero.name ? 'perk-selected' : ''}" data-hero="${item.hero.name}"><div class="name-stack small"><span class="hero-emoji hero-emoji-sm" role="img" aria-label="${item.hero.name}">${item.hero.emoji}</span><div class="hero-name-under">${item.hero.name}</div></div><h3>${item.hero.role}</h3><div class="reason-line">${item.summary}</div><div class="score-pill">Score ${item.score.toFixed(1)}</div></article>`;
}
...
function scoreHero(hero, enemies, map) {
  ...
  enemies.forEach(enemyName => {
    ...
    if (!enemy) return;
    const heroCountersEnemy = hero.strong_against.includes(enemyName);
    const enemyWeakToHero = enemy.weak_against.includes(hero.name);
    if (heroCountersEnemy && enemyWeakToHero) {
      score += 2.0;
      beats.push(enemyName);
    } else if (heroCountersEnemy || enemyWeakToHero) {
      score += 1.5;
      beats.push(enemyName);
    }
  ...
  if (map) {
    ...
    score += (MAP_MODE_BONUS[mode]?.[key] || MAP_MODE_BONUS[mode]?.[hero.name] || 0) * 0.4;
    if (hero.role === 'Tank' && (mode === 'Hybrid' || mode === 'Escort')) score += 0.1;
    if (hero.role === 'Support' && (mode === 'Control' || mode === 'Flashpoint')) score += 0.1;
  }

  if (state.playerHero) {
    const player = HERO_BY_NAME[state.playerHero];
    if (player && player.role !== hero.role) score += 0.2;
    if (player && hero.strong_against.includes(player.name)) score += 0.2;
  }
...
function renderLookup() {
  ...
  dom.lookup.innerHTML = `
    ...
    ${hero.strategic_rationale ? `<article class="lookup-card">
      <h3>Strategic rationale</h3>
      <div class="lookup-bullets">
        <div class="lookup-bullet">${hero.strategic_rationale}</div>
      </div>
    </article>` : ''}`;
}
...
function renderPlayerInfo() {
  ...
  const hero = HERO_BY_NAME[state.playerHero];
  const strong = hero.strong_against.slice().sort((a, b) => a.localeCompare(b));
  const weak = hero.weak_against.slice().sort((a, b) => a.localeCompare(b));

  // Find the 2 biggest threats to ban — heroes that counter your pick the hardest
  const banTargets = HERO_DB
    .filter(h => h.name !== hero.name)
    .map(h => {
      let threat = 0;
      if (h.strong_against.includes(hero.name)) threat += 4;
      if (hero.weak_against.includes(h.name)) threat += 3;
      return { hero: h, threat };
    })
    .filter(item => item.threat > 0)
    .sort((a, b) => b.threat - a.threat || a.hero.name.localeCompare(b.hero.name))
    .slice(0, 2);

  const banHtml = banTargets.length
    ? banTargets.map(b => `
        <div class="player-ban-item">
          <span class="hero-emoji hero-emoji-xs" role="img" aria-label="${b.hero.name}">${b.hero.emoji}</span>
          <div>
            <strong>${b.hero.name}</strong>
            <span class="mini-tag enemy-tag">Ban</span>
          </div>
        </div>`).join('')
    : '<div class="lookup-bullet">No clear ban targets.</div>';

  dom.playerInfo.innerHTML = `
    <div class="player-hero-detail">
      <div class="lookup-mini">
        <span class="hero-emoji" role="img" aria-label="${hero.name}">${hero.emoji}</span>
        <div>
          <span class="mini-tag map-tag">${hero.role}</span>
      ...
      <div class="lookup-bullets">
        <div class="lookup-bullet"><strong>Counters:</strong> ${strong.join(', ')}</div>
        <div class="lookup-bullet"><strong>Countered by:</strong> ${weak.join(', ')}</div>
        ${hero.strategic_rationale ? `<div class="lookup-bullet"><strong>Why:</strong> ${hero.strategic_rationale}</div>` : ''}
        ${hero.tips.slice(0, 3).map(t => `<div class="lookup-bullet"><strong>Tip:</strong> ${t}</div>`).join('')}
      </div>
      <div class="player-ban-section">
        <h4>Ban these against your ${hero.name}</h4>
        <div class="player-ban-list">${banHtml}</div>
      </div>
    </div>`;
}
